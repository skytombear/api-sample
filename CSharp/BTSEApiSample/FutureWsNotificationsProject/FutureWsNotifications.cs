...
namespace FutureWsNotificationsProject;

public class FutureWsNotifications
{
    ...
    private async Task OnOpenAsync()
    {
        ...
        var authPayload = new WsSubscriptionRequestEntity("authKeyExpires", new[]
        {
            headers["request-api"], headers["request-nonce"], headers["request-sign"]
        });
        await SendAsync(JsonSerializer.Serialize(authPayload));

        var subscribe = new WsSubscriptionRequestEntity("subscribe", new[] { "notificationApiV4" });
