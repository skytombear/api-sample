...
namespace FutureWsOrderbookSnapshotProject;

public class FutureWsOrderbookSnapshot
{
    ...
    {
        var subscribe = new WsSubscriptionRequestEntity("subscribe", new[] { "snapshotL1:BTC-PERP" });
        await SendAsync(JsonSerializer.Serialize(subscribe));
    }

    public async Task StartListeningAsync()
    {
        var wsUri = new Uri(Utils.GetOssFuturesWsUrl());
        try
        {
