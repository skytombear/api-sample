...
public class SpotAmendOrder
{
    private readonly HttpClient _httpClient = new();

    public async Task<object> AmendOrderAsync(SpotAmendOrderRequestEntity requestEntity)
    {
        const string endpoint = $"/api/{ApiConstants.SpotApiVersion}/order";
        var requestBody = JsonSerializer.Serialize(requestEntity);
        var request = new HttpRequestMessage(HttpMethod.Put, Utils.GetSpotFullUrl(endpoint))
        {
